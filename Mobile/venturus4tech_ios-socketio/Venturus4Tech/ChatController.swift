...
class ChatController : UIViewController, UITableViewDataSource {
    ...
    func getRequest() {
        ...
        let task = URLSession.shared.dataTask(with: url!) {(data, response, error) in
            ...
            if let asMsg = json as? [[String: Any]]  {
                self.msgs.append(contentsOf: asMsg)
                self.updateTableView()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let msgAuthor = msg["author"] as? String
        
        cell.message.text = msgText
        cell.username.text = msgAuthor
        cell.dateLabel.text = "\(msg["sent"] as! String)"
        cell.userInitial.text = "\(msgAuthor!.characters.first!)"
